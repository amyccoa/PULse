...
import sys
import time
from collections import deque, defaultdict #Amy default python add-in. 
...
### Amy , i indexed everything first, this should speed up the code but also make it more safe if you ever gonne parse millions of lines for your research. 
...
### Amy up and downstream are combined. 
if __name__ == "__main__":
    in_faa, in_marker, in_cazyme, in_pfam, outfi = sys.argv[1:6]
    start = time.perf_counter()

    relevant_pfams = ["SusD", "TonB", "CarboxypepD_reg", "TPR", "Glyco_hydro", "HTH_18", "SASA", "HisKA", "AraC", "DUF"]
        ...
        for marker_line in marker_lines:
            ...
            for i, (gene_id, start, end, header) in enumerate(parsed_faa):
                if gene_id == M_susD_ID or gene_id == M_tonB_ID:
                    found = True
                    down = find_nearby_genes(parsed_faa, i, "down", 500, 10, cazy_dict, pfam_dict, relevant_pfams)
                    up = find_nearby_genes(parsed_faa, i, "up", 500, 10, cazy_dict, pfam_dict, relevant_pfams)

                    ou.write(f"Marker: {gene_id}, Start: {marker_start}, End: {marker_end}\n")
                    ou.write("  Downstream:\n")
                    for d in down:
                        ou.write(f"\t{d}\n")
                    ou.write("  Upstream:\n")
                    ...
                    log(f"Processed {gene_id}: {len(up)} upstream, {len(down)} downstream results.")
                    break
    ...
    log(f"Completed annotation in {end - start:.2f} seconds.")
